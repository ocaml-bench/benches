...
# Opt-in, not part of `check`: nothing in the default path may need a running-ng
# checkout. RUNNING_NG_CONFIG overrides the config path.
...
# Input data generated by the *.build.deps.sh scripts: runtime-independent, shared
# by every runtime in a sweep, and slow to regenerate.
clean-data:
