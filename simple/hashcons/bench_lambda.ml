...
let quicksort =
  (* sandmark/running-ng adaptation: run cwd is a temp dir, so the data file
     path is taken from $QUICKSORT_TERM (absolute) when set, else the upstream
     relative default for standalone use. *)
  let path = match Sys.getenv_opt "QUICKSORT_TERM" with
    | Some p -> p
    | None -> "quicksort.term" in
...
let eval_nat iter init = function
  | {node=Abs {node=Abs t;_};_} (* [s,z]t *) ->
      let rec eval_rec = function
        | (* z *) {node=Ref 0;_} -> init
        | (* (s u) *) {node=App ({node=Ref 1;_}, u);_} -> iter (eval_rec u)
...
let eval_list_of_nats = function
  | {node=Abs {node=Abs t;_};_} (* [c,n]t *) ->
      let rec lrec = function
        | (* n *)       {node=Ref 0;_}                   -> []
        | (* (c x l) *) {node=App ({node=App ({node=Ref 1;_}, x);_}, l);_} ->
...
let () =
  Random.init 89;
  let l0 = List.init !n (fun _ -> Random.int !n) in
  let l1 = List.sort Stdlib.compare l0 in
  assert (normal_list (app (quicksort, list l0)) = l1);
