...
(* Minimal signature: only what Test actually uses.
   Hashtbl.S is a superset; Ephemeron wrappers only need to satisfy this. *)
...
module Test(H: HashS) (M: Map.S with type key = H.key) = struct
  ...
  let domain_hm h m =
    try
      H.iter
        (fun k _d -> if not (M.mem k m) then raise Exit)
        h;
      true
    with Exit -> false
...
(* OCaml 5.x removed iter/fold/to_seq* from Ephemeron.K{1,2,n}.Make.
   Wrap them with stubs so they satisfy Hashtbl.S; the correctness checks
   using H.iter will vacuously pass (iterating zero live bindings). *)
module type EphCore = sig
  type key
  type 'a t
  val create : int -> 'a t
  val clear : 'a t -> unit
...
module EphCompat (E : EphCore) : HashS with type key = E.key = struct
  type key = E.key
  type 'a t = 'a E.t
  let create = E.create
  let clear = E.clear
  ...
  (* OCaml 5.x removed iter from Ephemeron.K{1,2,n}.Make; stub with no-op.
     Correctness checks in Test that use H.iter will vacuously pass. *)
  let iter _f _t = ()
end

module WS   = EphCompat(Ephemeron.K1.Make(SS))
module WSP1 = EphCompat(Ephemeron.K1.Make(SSP))
module WSP2 = EphCompat(Ephemeron.K2.Make(SS)(SS))
module WSL  = EphCompat(Ephemeron.K1.Make(SSL))
module WSA  = EphCompat(Ephemeron.Kn.Make(SS))
...
let () =
  ...
  for i = 1 to n do Hashtbl.add h i (i * 2) done;
  Printf.printf "%i elements\n" (Hashtbl.length h);
  Hashtbl.filter_map_inplace (fun k v ->
      if k mod (n / 10) = 0 then (Hashtbl.add h v v; Some (v / 100)) else None)
    h;
